...
1. Mac 与手机在 **同一 Wi‑Fi**（避免访客网络隔离）。
2. 手机已允许 **本地网络 / 网络发现**（iOS：设置 → TreeletHub；Android：按系统提示）。
3. Mac 上 TreeletHub 在运行，配对码仍有效。
4. 关闭 VPN 后重试（Android 上较常见）。
5. Mac **重新生成配对码** 后，需用新码重连。
...
配对、布局与遥控默认 **仅在局域网**。详见 [隐私政策](https://treelet.us/treelethub/treelet-hub-privacy.html)。

## 如何反馈问题？

在 [GitHub Issues](https://github.com/shuzhzh/TreeletHub/issues) 提交，注明 macOS / iOS / Android 版本与复现步骤。
