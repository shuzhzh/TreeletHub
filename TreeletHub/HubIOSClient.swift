...
final class HubIOSClient: ObservableObject {
    ...
    /// Mac 在 layout 中报告的订阅是否有效（Watch 桥接仍会转发；iOS 自身不再做订阅/多页门控）。
    ...
    /// 与另一格交换位置（由 Mac 更新权威状态并广播 layout）；可跨页。
    func reorder(page: Int, from: Int, toPage: Int? = nil, to: Int) {
        guard phase == .paired else { return }
        let pageTo = toPage ?? page
        guard (0..<9).contains(from), (0..<9).contains(to) else { return }
        if page == pageTo, from == to { return }

        // 乐观本地交换，避免等 Mac layout 回传前图标卡住。
        applyLocalSwap(page: page, from: from, toPage: pageTo, to: to)

        let env = HubWireEnvelope(
            op: HubWireEnvelope.opReorder,
            page: page,
            from: from,
            to: to,
            pageTo: pageTo == page ? nil : pageTo
        )
        guard let data = try? HubWireCodec.encodeLine(env) else { return }
        connection?.send(content: data, isComplete: false, completion: .contentProcessed { _ in })
    }

    private func applyLocalSwap(page pageA: Int, from: Int, toPage pageB: Int, to: Int) {
        guard let indexA = pages.firstIndex(where: { $0.id == pageA }),
              let indexB = pages.firstIndex(where: { $0.id == pageB })
        else { return }
        var next = pages
        func remapped(_ source: HubSlotConfig, id: Int) -> HubSlotConfig {
            HubSlotConfig(
                id: id,
                kind: source.kind,
        ...
        if indexA == indexB {
            var slots = next[indexA].slots
            let a = slots[from]
            let b = slots[to]
            slots[from] = remapped(b, id: from)
            slots[to] = remapped(a, id: to)
            next[indexA].slots = slots
        } else {
            var slotsA = next[indexA].slots
            var slotsB = next[indexB].slots
            let a = slotsA[from]
            let b = slotsB[to]
            slotsA[from] = remapped(b, id: from)
            slotsB[to] = remapped(a, id: to)
            next[indexA].slots = slotsA
            next[indexB].slots = slotsB
        }
        pages = next
        layoutApplyEpoch &+= 1
    }
